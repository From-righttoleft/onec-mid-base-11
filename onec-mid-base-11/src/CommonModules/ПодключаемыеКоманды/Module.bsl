...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	//++ ООО АБС, Лесков Р.А., 23.08.24, Вызов процедуры добавления реквизитов на форму
	
	ДобавитьРеквизитыНаФорму(Форма);
...
Процедура ДобавитьРеквизитыНаФорму(Форма) 
	
	//++ ООО АБС, Лесков Р.А., 23.08.24, добавляет реквизиты и конманды на форму документов  
	
	КоличествоСимоволов = СтрДлина(Форма.ИмяФормы) - СтрНайти(Форма.ИмяФормы, ".", НаправлениеПоиска.СКонца);	
	ТипФормы = Прав(Форма.ИмяФормы,КоличествоСимоволов); 
	
	Если ТипФормы <> "ФормаДокумента" Тогда
		Возврат;
	КонецЕсли;
	          	
	Если Форма.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" ИЛИ 
			Форма.ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаДокумента" ИЛИ
			Форма.ИмяФормы = "Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента" Тогда
		
		ЭлементКонтактоеЛицо = Форма.Элементы.Вставить("КонтактноеЛицо", Тип("ПолеФормы"), Форма.Элементы.ГруппаШапкаПраво);
		ЭлементКонтактоеЛицо.Вид = ВидПоляФормы.ПолеВвода;
		ЭлементКонтактоеЛицо.ПутьКДанным = "Объект.Д_КонтактноеЛицо"; 

	ИначеЕсли Форма.ИмяФормы = "Документ.ОплатаПоставщику.Форма.ФормаДокумента" ИЛИ 
		   Форма.ИмяФормы = "Документ.ОплатаОтПокупателя.Форма.ФормаДокумента" Тогда
		
		ЭлементКонтактоеЛицо = Форма.Элементы.Вставить("КонтактноеЛицо", Тип("ПолеФормы"),,Форма.Элементы.Договор);
		ЭлементКонтактоеЛицо.Вид = ВидПоляФормы.ПолеВвода;
		ЭлементКонтактоеЛицо.ПутьКДанным = "Объект.Д_КонтактноеЛицо";
	...
	Если Форма.ИмяФормы = "Документ.ЗаказПокупателя.Форма.ФормаДокумента" Тогда 
		...
		ЭлементСогласованнаяСкидка = Форма.Элементы.Добавить("СогласованнаяСкидка", Тип("ПолеФормы"), Форма.Элементы.ГруппаСкидка);
		...
		КнопкаКомандыПересчитать = Форма.Элементы.Добавить("ПересчитатьТаблицу", Тип("КнопкаФормы"), Форма.Элементы.ГруппаСкидка); 
